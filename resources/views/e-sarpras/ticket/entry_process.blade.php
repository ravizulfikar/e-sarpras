...
@section('title', $pages['entry_process']['title'] ?? '-' )
...
<h3>{{ $pages['entry_process']['title']  ?? '-' }} </h3>
@endsection

@section('breadcrumb-items')
<li class="breadcrumb-item"><a href="{{ route($pages['dashboard']) }}">Dashboard</a></li>
<li class="breadcrumb-item"><a href="{{ route($pages['entry']['url']) }}">{{ $pages['entry']['title'] ?? '-' }}</a></li>
<li class="breadcrumb-item active">{{ $pages['entry_process']['title'] ?? '-' }}</li>
@endsection
	...
	@include('layouts.simple.spinner')
		...
		<div class="row">
			...
			<div class="col-xl-12">
				<div class="card">
					<div class="card-body">
						<form class="needs-validation" novalidate="" method="POST" action="{{ route($pages['entry_process']['update'], $data->id) }}" autocomplete="off" id="submitForm">
