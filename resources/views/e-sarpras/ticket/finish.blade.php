...
@section('title', $pages['finish']['title'] ?? '-' )
					...
					<table id="example" class="table table-bordernone table-hover table-striped" style="width:100%">
						...
						@foreach($data as $ticket)
							<tr>
								...
								<td class="va-middle">
									<a href="{{ route($pages['finish']['view'], $ticket) }}" class="btn btn-outline-info btn-xs"><i class="fa fa-eye"></i></a>
									{{-- <form method="POST" action="{{ route($pages['entry']['update'], $ticket) }}">
										@method('PUT')
										@csrf
										<button type="submit" class="btn btn-success btn-small">Process</button>
									</form> --}}
									{{-- <a href="{{ route($pages['finish']['view'], $ticket) }}" class="btn btn-outline-success btn-xs"> Update</a> --}}
									<a href="#" class="btn btn-outline-danger btn-xs remove" data-action="{{ route($pages['destroy'], $ticket) }}" data-id="{{$ticket}}"><i class="fa fa-trash"></i></a>
								</td>
							</tr>
						@endforeach
						</tbody>
