									...
									<table id="table_troubleshooting" class="table table-bordernone table-hover table-striped " style="width:100%">
										...
										@foreach($data["troubleshooting"] as $troubleshooting)
											<tr>
												...
												<td>{!! Location($troubleshooting->location, 'city') !!} - {!! Location($troubleshooting->location, 'unit') !!}</td>
												<td>{{ (!empty($troubleshooting->SignerTickets) ? $troubleshooting->SignerTickets->signer : '-') }}</td>
												<td>@if($troubleshooting->SignerTickets->sign != null)<i class="fa fa-check-square-o text-success"></i>@else<i class="fa fa-minus-square-o text-danger"></i>@endif {!! StatusBadge($troubleshooting->status) !!}</td>
												<td class="va-middle">
													<a href="#" data-action="{{ route($pages['reset'], $troubleshooting) }}" data-id="{{ $troubleshooting->id }}" data-method="PUT" class="btn btn-outline-danger btn-xs update">Reset</a>
													<a href="{{ route($pages['show']['url'], $troubleshooting) }}" class="btn btn-outline-info btn-xs">View</a>
												</td>
											</tr>
										@endforeach
										</tbody>
										...
										<table id="table_monitoring" class="table table-bordernone table-hover table-striped " style="width:100%">
											...
											@foreach($data["monitoring"] as $monitoring)
												<tr>
													...
													<td>{!! Location($monitoring->location, 'city') !!} - {!! Location($monitoring->location, 'unit') !!}</td>
													<td>{{ (!empty($monitoring->SignerTickets) ? $monitoring->SignerTickets->signer : '-') }}</td>
													<td>@if($monitoring->SignerTickets->sign != null)<i class="fa fa-check-square-o text-success"></i>@else<i class="fa fa-minus-square-o text-danger"></i>@endif {!! StatusBadge($troubleshooting->status) !!}</td>
													<td class="va-middle">
														<a href="#" data-action="{{ route($pages['reset'], $monitoring) }}" data-id="{{ $monitoring->id }}" data-method="PUT" class="btn btn-outline-danger btn-xs update">Reset</a>
														<a href="{{ route($pages['show']['url'], $monitoring) }}" class="btn btn-outline-info btn-xs">View</a>
													</td>
												</tr>
											@endforeach
											</tbody>
...
<script>
	$(function() {
		// var table = $('#example');
		var table2 = $('#table_monitoring').DataTable({
			responsive: true,
			columnDefs: [
				{ searchable: false, orderable: false, width: 10, targets: 0, className: 'text-center' },
				{ width: 60, targets: 1 },
		...
		table2.on( 'order.dt search.dt', function () {
			...
			table2.cells(null, 0, {search:'applied', order:'applied'}).every( function (cell2) {
				this.data(i2++);
			} );
		} ).draw();
...
<script>
	$(function() {
		// var table = $('#example');
		var table3 = $('#table_server').DataTable({
			responsive: true,
			columnDefs: [
				{ searchable: false, orderable: false, width: 10, targets: 0, className: 'text-center' },
				{ width: 60, targets: 1 },
