...
{
	public function mainTicket($type, $verif)
    {
        return Ticket::with('SignerTickets')->whereIn('type', $type)->whereStatus('finish')->whereIn('verification', $verif)->orderBy('verification', "DESC")->get();
    }

    public function reSign($ticket)
    {
        $data = SignerTicket::whereTicketId($ticket->id)->first();
        $data->update(['sign' => '']);
        return redirect()->back()->with('success', "Ticket successfully reset to re sign client !");
    }

    public function mainReport($verif)
    {
        return Report::with('ReportDescription', 'ReportPicture')->whereIn('verification', $verif)->orderBy('verification', "DESC")->get();
    }

    public function showReport($field, $id)
    {
